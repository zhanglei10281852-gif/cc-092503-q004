...
- 谱系操作账本：每个冻干/研磨/分装操作保存处理前后单位、换算规则快照、可解释损耗与操作版本，更正只追加新版本，历史永不改写。
- 换算规则：按规则编码版本化登记，重复提交幂等回放，旧版本始终可复算。
- 谱系追溯与汇总：任意节点可向上逐级追溯来源操作，向下汇总存量、累计消耗、损耗与销毁，并归一到锚点单位核对整条谱系守恒。
- 跨层报告：自动标出断链、循环、重复编码与超出容差的步骤，摘要与问题排序对相同数据完全稳定（含确定性摘要 digest）。
